...
class EMLExtractorGUI:
    ...
    def create_widgets(self):
        ...
        ttk.Label(input_frame, text="Input Folder:").grid(row=0, column=0, sticky="w", pady=5)
    ...
    def start_extraction(self):
        ...
        thread = threading.Thread(target=self.process_emails, args=(input_path, output_path))
        thread.daemon = True
        thread.start()
    ...
    def process_emails(self, input_folder, output_folder):
        ...
        try:
            ...
            if not processor.input_folder.exists():
                self.progress_queue.put(("error", f"✗ Error: Folder not found - {processor.input_folder}"))
                return
            ...
            if not eml_files:
                self.progress_queue.put(("error", f"✗ No .eml files found in {processor.input_folder}"))
                return
            
            total_files = len(eml_files)
            self.progress_queue.put(("info", f"\n{'='*70}"))
            self.progress_queue.put(("info", f"BATCH EML EXTRACTION"))
            self.progress_queue.put(("info", f"{'='*70}"))
            self.progress_queue.put(("info", f"Input folder: {processor.input_folder.absolute()}"))
            self.progress_queue.put(("info", f"Output folder: {processor.output_base.absolute()}"))
            ...
            for idx, eml_file in enumerate(eml_files, 1):
                ...
                self.progress_queue.put(("info", f"[{idx}/{total_files}] Processing: {eml_file.name}"))
                ...
                try:
                    ...
                    # Print summary
                    self.progress_queue.put(("info", f"From: {info['headers']['From']}"))
                    self.progress_queue.put(("info", f"Subject: {info['headers']['Subject']}"))
                    self.progress_queue.put(("info", f"Date: {info['headers']['Date']}"))
                    self.progress_queue.put(("success", f"Plain text body: {'✓' if info['has_plain'] else '✗'}"))
                    self.progress_queue.put(("success", f"HTML body: {'✓' if info['has_html'] else '✗'}"))
                    self.progress_queue.put(("info", f"Attachments: {info['attachment_count']}"))
                    self.progress_queue.put(("success", f"✓ Extracted to: {output_folder_path.name}/"))
                ...
                self.progress_queue.put(("status", f"Processing {idx}/{total_files}..."))
                self.progress_queue.put(("stats", f"Successful: {successful} | Failed: {failed}"))
...
def main():
    ...
    def on_closing():
        if app.is_processing:
            if messagebox.askokcancel("Quit", "Extraction is in progress. Are you sure you want to quit?"):
